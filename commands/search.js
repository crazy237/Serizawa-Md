...
console.log("text : " , textt)
    ...
    if (!url) return send(citel,"*_Stp, donnez un lien pour la recherche!_*");
     send(citel,url[0]);
...
catch (err) { return send(citel,"```Erreur de recherche```")}
        }
    )
...
smd({
    ...
    async(Suhail, citel, text) => {
 if(!text) return await citel.reply('Donne un numéro sans le signe + . Exemple: .iswa 2376942682xx')
        var inputnumber = text.split(" ")[0]
        if (!inputnumber.includes('x')) return citel.reply(`*You did not add x*\nExemple: iswa 2376942682xx  \n ${Config.caption}`)
        citel.reply(`*Recherche du compte whatsapp dans la base de données...* \n ${Config.caption}`)
...
smd({
    ...
    async(Suhail, citel, text) => {
if(!text) return await citel.reply('Donne un numéro sans le signe +. Exemple: .nowa 2376942682xx')
const inputNumber = text.split(" ")[0]
if (!inputNumber.includes('x')) return citel.reply(`*You did not add x in number.*\nExemple: ${prefix}nowa 23769426821xx  \n ${Config.caption}`)
...
let nobio = `\n*『 WhatsApp Account With No Bio』* \n`;
 let nobios='';
let nowhatsapp = `*『 Numéros sans comptes whatsapp 』* \n\n`;
for (let i = 0; i < randomxx; i++) 
{
