...
//                  AI  CHAT  COMMAND
//---------------------------------------------------------------------------
smd({ pattern: "chat",desc: "chat with an AI",category: "ai",use: '<Hey, Crazy237>', filename: __filename,},async(Aviator, msg,text) => { return msg.reply(await aiResponce(msg, "chat", text)); }
)


//---------------------------------------------------------------------------
smd({pattern: "gpt",desc: "chat with an AI",category: "ai",use: '<Hey, Crazy237>',filename: __filename,},
...
//---------------------------------------------------------------------------
smd({pattern: "dalle",alias : ['dall','dall-e'],desc: "chat with an AI",category: "ai", use: '<Hey, Crazy237>',filename: __filename,},
    async(Aviator, msg,text) =>{
  if ( Config.OPENAI_API_KEY=='' ||  !Config.OPENAI_API_KEY.startsWith('sk') ) return msg.reply('```You Dont Have OPENAI API KEY \nPlease Create OPEN API KEY from Given Link \nhttps://platform.openai.com/account/api-keys\nAnd Set Key in Heroku OPENAI_API_KEY Var```')
