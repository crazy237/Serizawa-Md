...
async(Suhail, msg, text,{ isCreator }) => {
  ...
  if (text.toLowerCase().startsWith("on") || text.toLowerCase().startsWith("act") || text.toLowerCase().startsWith("enable") ) {
    if (checkinfo.antidemote == 'true') return await msg.send("*Anti_Demote déjà actif!*")
    await sck.updateOne({ id: msg.chat }, { antidemote : 'true' });
    return await msg.send("*Anti_Demote activé avec succès! _Personne n'est démi desormais_.*")
  }else if (text.toLowerCase().startsWith("off") || text.toLowerCase().startsWith("deact") || text.toLowerCase().startsWith("disable") ) {
    if (checkinfo.antidemote == 'false') return await msg.send("*Anti_Demote déjà inactif!*")
    await sck.updateOne({ id: msg.chat }, { antidemote : 'false' });
    return await msg.send("*Anti_Demote désactivé avec succès!*")
  }
  else return await msg.reply(`*Hey, Please ajoute entre "On" ou "Off".* \n*_Pour arreter de revoquer les admins!_*`)
});
//---------------------------------------------------------------------------
smd({
    pattern: "antipromote",
...
async(Suhail, msg, text,{ isCreator }) => {
  ...
  let checkinfo = await sck.findOne({ id : msg.chat })  || await sck.new({ id: msg.chat});
  if (text.toLowerCase().startsWith("on") || text.toLowerCase().startsWith("act") || text.toLowerCase().startsWith("enable") ) {
    if (checkinfo.antipromote == 'true') return await msg.send("*Anti_Promote déjà actif!*")
    await sck.updateOne({ id: msg.chat }, { antipromote : 'true' });
    return await msg.send("*Anti_Promote activé avec succès! _Personne n'est nommé desormais_.*")
  }else if (text.toLowerCase().startsWith("off") || text.toLowerCase().startsWith("deact") || text.toLowerCase().startsWith("disable") ) {
    if (checkinfo.antipromote == 'false') return await msg.send("*Anti_Promote déjà inactif!*")
    await sck.updateOne({ id: msg.chat }, { antipromote : 'false' });
    return await msg.send("*Anti_Promote désactivé avec succès!*")
  }
  else return await msg.reply(`*Hey, Please ajoute entre "On" ou "Off "On" And "Off".* \n*_Pour arreter de nommer les admins_*`)
});
//---------------------------------------------------------------------------
smd({
    pattern: "pdm",
...
async(Suhail, msg, text,{ isCreator }) => {
  ...
  let checkinfo = await sck.findOne({ id : msg.chat })  || await sck.new({ id: msg.chat});
  if (text.toLowerCase().startsWith("on") || text.toLowerCase().startsWith("act") || text.toLowerCase().startsWith("enable") ) {
    if (checkinfo.pdm == 'true') return await msg.send("*Alerte Promote/Demote ajoutée avec succès*")
    await sck.updateOne({ id: msg.chat }, { pdm : 'true' });
    return await msg.send("*Alerte Promote/Demote activé avec succès!*")
  }else if (text.toLowerCase().startsWith("off") || text.toLowerCase().startsWith("deact") || text.toLowerCase().startsWith("disable") ) {
    if (checkinfo.pdm == 'false') return await msg.send("*Alerte Promote/Demote déjà inactif!*")
