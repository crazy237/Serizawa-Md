...
/*smd({
        ...
        async(Suhail.bot, citel, text) => {
          if(!text) return citel.reply(`*_Donnez moi le lien Facebook_*`);
...
smd({pattern: "tiktok",alias :  ['tt','ttdl'],desc: "Downloads Tiktok Videos Via Url.",category: "downloader", filename: __filename, use: '<add tiktok url.>'},
        ...
        async(Suhail, citel, text) => {
 if(!text) return await citel.reply(`*Hey,Donne moi le lien TikTok*\n*_Ex .tiktok https://www.tiktok.com/@thisisdamso/video/_*`);
 let txt = text ? text.split(" ")[0]:'';
 if (!/tiktok/.test(txt)) return await citel.send(`*Hey stp, Donne moi un lien valide TikTok!*`);
 ...
 else return await citel.send("Erreur") 
})
//---------------------------------------------------------------------------
/*
smd({
        ...
        async(Suhail.bot, citel, text) => {
if(!text) return await citel.reply(`*Hey, Donne moi un lien TikTok*\n*_Ex .tiktok https://www.tiktok.com/@thisisdamso/video_*`);
let txt = text ? text.split(" ")[0]:'';
const ttdl =  require("tiktok-video-downloader");
if (!txt.includes("tiktok.com")) return  citel.reply(`*Hey stp, Donne moi un lien valide TikTok!*`);
try {
...
} catch (error) {return citel.reply("Erreur") }
...
//console.log(info);
  ...
  .catch(err => {citel.reply("Erreur, Video Not Found\n *Donne moi un lien valide*");
			console.error(err);})
...
smd({
        ...
        async(Suhail, citel, text) => {
        if(!text )return citel.reply("*_Donne moi le nom de l\'appli_*");
	...
	else return citel.send("*_APK non trouvée, réessayez_*");
	
	
	const apkSize = parseInt(data.size);
	if(apkSize > 100) return citel.send(`❌ Taille du fichier supérieur à 150Mb.`);
...
async(Suhail, citel, text) => {
if (!text) return citel.send('Hey, donne moi un lien Google Drive') 
if (!(text && text.match(/drive\.google/i))) citel.send('Hey, donne moi un lien google drive valide')
...
try {
	...
	{ 
	        ...
	        let buttonMessage = 
		{
			...
			caption : "\t  *TELECHARGEMENT GOOGLE DRIVE*  \n" + data
		}
	        return await Suhail.bot.sendMessage(citel.chat,buttonMessage, { quoted: citel })
	})
...
smd({pattern: "gitclone",desc: "Downloads apks  .",category: "downloader",filename: __filename,use: '<git url>', },
        async(Suhail, citel, text) => {
	if (!text) return await citel.send('*Donnez un lien github, Ex:- _.gitclone https://github.com/Crazy237/Serizawa-Md_*') 
    const regex = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    if (!regex.test(text) ) return await citel.send('*Hey stp, donne un lien valide*');
...
smd({pattern: "tts",desc: "text to speech.",category: "downloader",filename: __filename,use: '<Hii,this is Suhail>',},
        async(Suhail, citel, text) => {
            if (!text && !citel.quoted) return citel.reply(`*Stp donne moi un texte*\n *_Example : .tts Vie sur nous, Crazy237._*`);
...
async(Suhail, citel, text) => {
  ...
  if (!text) return citel.reply(`Example : ${prefix}video Damso Sentimental`);
  ...
  try{
    ...
    if( VidTime  >= videotime) return await citel.reply(`*_Impossible de télécharger, fichier trop lourd_*`);
    ...
    let buttonMessage = { video: fs.readFileSync(randomName),mimetype: 'video/mp4',caption: "  Voici ta vidéo\n" + Config.caption ,height: 496, width: 640,}
  ...
  //  
    ...
    try{
      ...
      if( info.duration  >= videotime) return await citel.reply(`*_Impossible de télécharger, fichier trop lourd_*`);
      let meta = { type:"video", quality: info.pref_Quality,}
      let file = await yt.download(vid[1] , meta )
      let thumb = await botpic();
      file ? await Suhail.bot.sendMessage(citel.chat, { video: {url : file },caption: "  *Voici ta vidéo*\n" + Config.caption ,mimetype: 'video/mp4',jpegThumbnail: log0,height: 496, width: 640 }) :  await citel.send("Video not Found"); 
      try{fs.unlinkSync(`${file}`)}catch{}
    }catch(err) {console.log("ytdl Download video erreur:", e); console.log("Youtubei Video Download Erreur :" , err);return await citel.error(`${err} \n\ncmdName : video` )   }
...
async(Suhail, citel, text) => {
  ...
  try {
    ...
    vid = ytIdRegex.exec(urlYt) || [];
    let info =await yt.getInfo(vid[1]);
    if( info  && info.duration  >= videotime) return await citel.reply(`*_Impossible de télécharger, durée trop longue\_*`);
    ...
    try{fs.unlinkSync(file)}catch{}
  }catch (e) { console.log(" Play erreur, "  , e); return citel.error(`${e} \n\ncmdName : play`) }
})
...
smd({pattern: "pint",desc: "Downloads image from pinterest.",category: "downloader",filename: __filename,use: '<text|image name>',},
        async(Suhail, citel, text) => {
            ...
            try {
                ...
                return Suhail.bot.sendMessage(citel.chat, buttonMessage, {  quoted: citel })
            } catch (e) {  return citel.reply("Hey, donne moi un nom. Ex .pint Damso")  }
        })
    //---------------------------------------------------------------------------
smd({pattern: "mediafire",alias :['mf','mfire'],desc: "Downloads media from Mediafire.",category: "downloader",filename: __filename,use: '<url of mediafire>',},
        async(Suhail, citel, text) => {
            ...
            if (!text.includes("mediafire.com")) return citel.reply(`lien invalide`);
            let isUrl=text;
            const baby1 = await mediafire(isUrl);
	
	if(!baby1.length) return citel.reply(`rien trouvé`);
	const apkSize = parseInt(baby1[0].size);
	if(apkSize > 100) return citel.reply(`❌ Taille du fichier supérieur à 150Mb.`);
	
let result4 = ` *Téléchargement Mᴇᴅɪᴀғɪʀᴇ*\n*Nᴀᴍᴇ* : ${baby1[0].nama}\n*Taille* :${baby1[0].size}\n*Mɪᴍᴇ* : ${baby1[0].mime}\n\n${Config.caption}`;
...
smd({pattern: "song", alias: ["audio"],desc: "Downloads audio from youtube.",category: "downloader",filename: __filename,use: '<give text>', },
        async(Suhail, citel, text) => {
                if (!text) return await citel.reply(`*_Hey, donne le titre d\'un song_*`);
                let yts = require("secktor-pack")
                let search = await yts(text);
                let i = search.all[1] ;
                let cap = "\t *---Yt recherche de song---*   \n\nTitre : " + i.title + "\nUrl : " + i.url +"\nDescription : " + i.timestamp +"\nVues : "+i.views +"\nTéléchargement : " +i.ago +"\nAuteur : "+i.author.name+"\n\n\nRepond par 1 pour Video \nRepond par 2 pour Audio" ;
...
async(Suhail, citel, text) => {
  ...
  let yts = require("secktor-pack");
  text = text ? text : citel.quoted && citel.quoted.text ? citel.quoted.text : ""
  if (!text) return citel.reply(`Exemple : ${prefix}yts WhatsApp Bot par Serizawa-Md_`);
  let search = await yts(text);
...
async(Suhail, citel, text) => {
  ...
  if (!text || !vid[0]) return await citel.reply(`❌Donnez moi un lien SVP`);
  try {
    let infoYt = await ytdl.getInfo(vid[0]);
    if(infoYt.videoDetails.lengthSeconds >= videotime) return citel.reply(`*_Impossible de télécharger, fichier trop lourd_*`);
    let titleYt = infoYt.videoDetails.title;
    let randomName = `./temp/${vid[1]}.mp4`;
    const stream = ytdl(urlYt, {filter: (info) => info.itag == 22 || info.itag == 18,}).pipe(fs.createWriteStream(`${randomName}`));
    await new Promise((resolve, reject) => {stream.on("error", reject);stream.on("finish", resolve); });
//let stats = fs.statSync(`${randomName}`);
    ...
    try{
      ...
      try{fs.unlinkSync(`${file}`)}catch{}
    }catch(err) { console.log("ytdl Download video erreur ytmp4 :", e);console.log("Youtubei Video Download Erreur ytmp4 :" , err);return await citel.error(`${err} \n\ncmdName : ytmp4` )   }
...
async(Suhail, citel, text) => {
      ...
      if (!text || !vid[0]) return await citel.reply(`❌Donnez moi un lien SVP`);
        try {
          let infoYt = await ytdl.getInfo(vid[0]);
          if(infoYt.videoDetails.lengthSeconds >= videotime) return citel.reply(`*_Impossible de télécharger, fichier trop lourdig_*`);
          let titleYt = infoYt.videoDetails.title;
          ...
          try{    
            ...
            console.log("ytdl Téléchargement yt audio erreur:", e);
            console.log("Erreur de téléchargement yt audio\n " , err); 
  ...
  var titleYt = vid[1],file = false
        try {   
          ...
          try{
            ...
            console.log("ytdl Download document erreur:", e);
            console.log("Error de téléchargement yt audio document\n " , err); 
            return await citel.error(`${err}\n\ncmdName: ytdoc`);
          }        
        }
  if(!file) return await citel.send("*_Hey, video not found_*")
  try{ 
                let yts = require("secktor-pack");
