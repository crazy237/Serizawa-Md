...
async(Suhail, msg, text,{ isCreator }) => {
    ...
    if(!text) return await msg.reply("*Donne la description que tu veux mettre*")
    ...
    if (!isBotAdmins) return await msg.reply(`*_Je ne suis pas Admin dans ce groupe, Idiot_*`); 
    if (!isAdmins) return msg.reply(tlang().admin);
    
    try {
        await Suhail.bot.groupUpdateDescription(msg.chat, text);
        msg.reply('*_✅Description changée avec succès.!_*') 
        return await Suhail.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
    } catch(e) { return await Suhail.bot.sendMessage(users , {text :"Erreur\nReason : " + e, } ,{quoted : msg})   }
}
)
//---------------------------------------------------------------------------
smd({
...
async(Suhail, msg, text,{ isCreator }) => {
    if (!msg.isGroup) return msg.reply(tlang().group);
    if(!text) return await msg.reply("*Hey, donne un texte pour le nom du groupe*")
    const groupAdmins = await getAdmin(Suhail.bot, msg)
    const botNumber = await Suhail.bot.decodeJid(Suhail.bot.user.id)
    const isBotAdmins = msg.isGroup ? groupAdmins.includes(botNumber) : false;
    const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
    ...
    try {
        ...
        msg.reply('*_✅Nom du groupe changé avec succès.!_*') 
        return await Suhail.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
    } catch(e) { return await Suhail.bot.sendMessage(users , {text :"_Erreur_\nRaison : " + e, } ,{quoted : msg})   }
}
)
    //---------------------------------------------------------------------------
...
smd({
    ...
    async(Suhail, msg, text,{isCreator}) => {
        ...
        if(!users)  return await msg.reply("*Hey, tag ou mentionne un membre*")
        await Suhail.bot.updateBlockStatus(users, "bloqué")
...
smd({
	    ...
	    let num = users.replace("@s.whatsapp.net","")
            await Suhail.bot.updateBlockStatus(users, "débloqué")
                .then((res) => msg.send(`*@${num} Débloqué avec succès..!*`,{mentions : [ users , ]}))
                .catch((err) => console.log(jsonformat(err)));
        }
    )
    //---------------------------------------------------------------------------
...
if (!isBotAdmins) return msg.reply("*_Je ne suis pas admin, du coups j\'ai pas le lien, Idiot_*");
var str1 = await Suhail.bot.groupInviteCode(msg.chat)
var str2 ="https://chat.whatsapp.com/"
var mergedString = `${str2}${str1}`;
return msg.reply("*Lien du groupe, Partagez SVP* \n*"+mergedString+"*");
  ...
  smd({
	...
	const botNumber = await Suhail.bot.decodeJid(Suhail.bot.user.id)
        const isBotAdmins =groupAdmins.includes(botNumber)
	if (!isBotAdmins) return msg.reply("*_Je ne suis pas admin, du coups je ne peux pas réinitialiser le lien_*");
	    
var code = await Suhail.bot.groupRevokeInvite(msg.chat)
return msg.reply("*_Lien du groupe réinitialisé_*");
...
`
    ...
    {	
        if (!msg.isGroup) return msg.reply(tlang().group);
	if(!text) return await msg.reply("*Ajouter le code du pays. Exemple: .kik 229*")
        const groupMetadata = msg.isGroup ? await Suhail.bot.groupMetadata(msg.chat).catch((e) => {}) : "";
	const groupAdmins = await getAdmin(Suhail.bot, msg)
        let isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) :  false  ;
        if (!isAdmins)
	{
		if(isCreator) msg.reply("*Hey Crazy237, tu n'es pas Admin ici*")
		else return msg.reply(tlang().admin);
	}
	let find = text.split(" ")[0].replace('+' , '');
	let error = '*Utilisateur non retiré, erreur* \n\t' ;
	...
	for (let i of users) { 
		...
		{ 
			...
			{
				iskikstart = true ;
				await msg.reply(`*_Kick tous les utilisateurs avec le numéro de pays ${find}_*`)
			}
			try { await Suhail.bot.groupParticipantsUpdate(msg.chat, [i.id], "remove"); hmanykik++ ;  }
			catch (e) { console.log("Erreur de Kicking : " , e) } 	
		}
	}
	if(hmanykik == 0) return await msg.reply(`*_Chance, Personne du pays ${find}_*`)
        else return await msg.reply(`*_Enfin, ${hmanykik.toString()} utilisateurs avec le code ${find} kické_*`)
})
//---------------------------------------------------------------------------
smd({
        pattern: "num",
    ...
    {	
        if (!msg.isGroup) return msg.reply(tlang().group);
	if(!text) return await msg.reply("*Donne moi un numéro de pays. Exemple: .num 229*")
        const groupMetadata = msg.isGroup ? await Suhail.bot.groupMetadata(msg.chat).catch((e) => {}) : "";
	const groupAdmins = await getAdmin(Suhail.bot, msg)
        const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) :  false  ;
        if (!isAdmins && !isCreator ) return msg.reply(tlang().admin);
	...
	let users = await groupMetadata.participants
	let nums = `*Liste des utilisateurs avec du ${find}*\n`
	let num = '';
	for (let i of users) {  if(i.id.startsWith(find)) num += i.id.split("@")[0] +"\n";   }
	if(!num) {nums =`*Aucun utilisateur du ${find}*` }
...
smd({
        ...
        async(Suhail.bot, msg, text) => {
            if (!msg.quoted) return reply(`Tag une video ou une image vue unique!`);
            ...
            if (/viewOnce/.test(mime)) {
                ...
                await msg.reply("Stp, tag une vue unique, Idiot");
            }
        }
    )*/
    ...
    smd({
        ...
        async(Suhail, msg, text ,{ isCreator }) => {	
            ...
            const isBotAdmins = msg.isGroup ? groupAdmins.includes(botNumber) : false;
            const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
	        if (!isBotAdmins) return msg.reply("*_Je ne suis pas Admin, du coups je peux pas nommer_*");
            if (!isAdmins) return msg.reply(tlang().admin);
            
            try {
                let users = msg.quoted ? msg.quoted.sender : msg.mentionedJid[0] ? msg.mentionedJid[0] : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
                if (!users) return await msg.send("*_Hey, tag ou mentionne un membre_*");
                await Suhail.bot.groupParticipantsUpdate(msg.chat, [users], "promote");
                await msg.send(`*Utilisateur nommé avec succès!*`)
            } catch(e) {
                console.log("Erreur de nomination : " , e )
...
//if (!isCreator) return msg.reply("```Only My Owner Can Use This Command```")
    ...
    const isBotAdmins = msg.isGroup ? groupAdmins.includes(botNumber) : false;
    const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
    if (!isBotAdmins) return await msg.reply(`*_Je ne suis pas admin, Idiot_*`); 
    if (!isAdmins) return msg.reply(tlang().admin);
    ...
    try {
        ...
        await msg.send(`*Membre démi avec succès!*`)
        return await Suhail.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
    } catch(e) {
        console.log("Erreur de denomination : " , e )
        await Suhail.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
...
smd({
	...
	//if (!isCreator) return msg.reply("*_Only My Owner Can Use This Command_*")
            ...
            try {
                let users = msg.quoted ? msg.quoted.sender : msg.mentionedJid[0] ? msg.mentionedJid[0] : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
                if (!users) return msg.send("*_Hey, tag ou mentionne un membre_*");
                await Suhail.bot.groupParticipantsUpdate(msg.chat, [users], "remove");
                await msg.send(`*Enfin, membre retiré avec succès!*`)
                return await Suhail.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
            } catch(e) {
                console.log("Erreur deKick : " , e )
                await Suhail.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
                return await msg.reply(tlang().botAdmin);
    ...
    smd({
        ...
        async(Suhail, msg, text) => {
            ...
            if (action.startsWith("close") || action.startsWith("mute") ) {
                await Suhail.bot.groupSettingUpdate(msg.chat, "announcement").then((res) => msg.reply(`*_GROUPE FERME!!!_*`)).catch((err) => msg.error(err));
            } else if (text.toLowerCase().startsWith("open")||text.toLowerCase().startsWith("unmute") ){
                await Suhail.bot.groupSettingUpdate(msg.chat, "not_announcement").then((res) => msg.reply(`*_GROUPE OUVERT!!!_*`)).catch((err) => msg.error(err));
            }
...
${listAdmin}
*▢ Description :*
   • ${groupMetadata.desc?.toString() || 'rien'}
   `
...
return await Suhail.bot.sendMessage(msg.chat,{text : tag ,mentions: [msg.sender, ...groupAdmins.map(v => v.id) ,]} ,)
	...
	//if (!isCreator) return msg.reply("```Only My Owner Can Use This Command```")
            ...
            let users = msg.quoted ? msg.quoted.sender : msg.mentionedJid[0] ? msg.mentionedJid[0] : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
            if (!users) return await msg.reply("*_Hey, donne un mebre._*");
            if(msg.sender == botNumber ){
                await Suhail.bot.groupParticipantsUpdate(msg.chat, [users], "add");
                await msg.send(`*Membre ajouté avec succès!*`)
                return await Suhail.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
            }else {
                await Suhail.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
                await Suhail.bot.sendMessage(users , {text : `Voici le lien du groupe\n @${msg.sender.split("@")[0]} Veux intégrer\n https://chat.whatsapp.com/${await Suhail.bot.groupInviteCode(msg.chat)} _ \n ------------ Join If YOu Feel Free?` ,mentions:[msg.sender,]} , {quoted : msg })
                return await msg.reply(`_Je ne peux pas le rajouter, Invitation envoyée_`)
            }
        }
    )
    //--------------------------------------------------------------------------- 
...
smd({
        ...
        async(Suhail, msg, text,{ isCreator, botNumber }) => {
          if (!msg.quoted) return msg.reply(`*_Stp tag un message!!!_*`);
          try{
            ...
            if (!msg.quoted.isBot ) {
                ...
                const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
                if (!isAdmins) return msg.reply(tlang().admin)
                if (!isBotAdmins) return msg.reply('Je ne peux pas supprimer ce message sans etre admin.')
    ...
    async(Suhail, msg, text,{isCreator}) => {
        if (!isCreator) return msg.reply(tlang().owner)
        if(!text) return await msg.reply(`*_Hey, Ajoute le texte au Broadcast dans tous les groupes_*`)
        let getGroups = await Suhail.bot.groupFetchAllParticipating();
        let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1]);
        let anu = groups.map((v) => v.id);
        await msg.send(`*_Broadcast envoyé  ${anu.length} Chat, Heure de fin ${ anu.length * 1.5} secondes_*`);

            let txt = `*--❗${tlang().title} Broadcast❗--*\n\n *🍀Message:* ${text}`;
let contextInfo = {forwardingScore: 999,isForwarded: true, externalAdReply: { title:'Serizawa-Md Broadcast',body:msg.pushName,renderLargerThumbnail: true,thumbnail: log0,mediaType: 1,mediaUrl: '',sourceUrl: gurl,showAdAttribution: true,},}
  ...
  try{
        ...
        return await msg.reply(`*Broadcast envoyé au ${anu.length} avec succès*`);
    }
)
