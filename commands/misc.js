...
smd({
 ...
 if (!msg.quoted) return msg.reply("*Tag un code pour exécuter*")
            try {
                const code = {
...
smd({
        ...
        async(Suhail, msg, text) => {
            if(!msg.quoted && !msg.mentionedJid) return await msg.reply(`*Tag ou mentionne un membre*`);
...
smd({
        ...
        async(Suhail, msg, match) => {
            if (!match) return msg.reply("*Quel type de membre tu veux?*");
            ...
            Suhail.bot.sendMessage(msg.chat, {
                text: `Le plus ${match} de nous c\'est ?' *@${pick.split("@")[0]}*`,
                mentions: [pick],
            }, {
                quoted: msg,
            });
...
smd({
        ...
        async(Suhail, msg, text) => {
            if (!text) return msg.reply('Donne moi un package name.📦')
...
smd({
        ...
        async(Suhail, msg, text) => {
            if (!text) return msg.reply(`Example : ${prefix}fliptext CrazyPrince`)
...
smd({
        ...
        async(Suhail, msg, text) => {
            if (!text) return msg.reply(`_Donne le lien de video ?_`);
            Suhail.bot.sendMessage(msg.chat, {
                video: {
                    url: text.split(" ")[0],
                },
...
smd({
        ...
        async(Suhail, msg, text,{ isCreator }) => {
            try {
                if (!text) return msg.reply(`Envoie un texte à coder.`);
...
smd({
        ...
        async(Suhail, msg, text,{ isCreator }) => {
            try {
                if (!text) return msg.reply(`Envoie un texte à decoder.`);
