...
if(isMongodb){ let groupid = ''
    ...
    async(Suhail, msg, text,{ isCreator }) => {
        ...
        if(!text.split(':')[1]) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`)
        //if(!Number.isInteger(text.split(':')[0])) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`);
        //if(!Number.isInteger(text.split(':')[1])) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`)
              let Group = await sck.findOne({ id: msg.chat })
                if (!Group) {
                    ...
                    return msg.reply(`_Muet ajouté à ${text} avec succès._`)     
                }      
    }
    )
   ...
   async(Suhail, msg, text,{ isCreator }) => {
       if (!isCreator) return msg.reply(tlang().owner)
       if(!msg.isGroup) return msg.reply(tlang().group)
       if(!text.split(':')[0]) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`)
      // if(!Number.isInteger(text.split(':')[0])) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`);
      // if(!Number.isInteger(text.split(':')[1])) return msg.reply(`Dans le format correct svp.\nEg: setmute ${prefix}22:00`)
             let Group = await sck.findOne({ id: msg.chat })
               if (!Group) {
                   await new sck({ id: msg.chat, unmute: text }).save()
                   return msg.reply('Mute ajouté.')
               } else {
                   await await sck.updateOne({ id: msg.chat }, { unmute:text })
                   return msg.reply(`_Unmute mis à jour pour ${text} avec succès._`)
   ...
   async(Suhail, msg, text,{ isCreator }) => {
       ...
       if(!msg.isGroup) return msg.reply(tlang().group)
             let Group = await sck.findOne({ id: msg.chat })
               if (!Group) {
                   return msg.reply('Pas de unmute pour ce groupe')
               } else {
                   await await sck.updateOne({ id: msg.chat }, { unmute:'false' })
                   return msg.reply('Unmute supprimé avec succès.')
