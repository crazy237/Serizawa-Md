...
if (global.sudo.includes(user)) return msg.reply("Ce numéro est déjà sudo");
    global.sudo += ',' + user ;
const headers = 
        {
...
if(!msg.quoted) return msg.reply(`*STP tag un membre*`);
let user = msg.quoted.sender.split('@')[0] ;
let  rm = ',' +user 
if (global.sudo.includes(rm)) global.sudo = global.sudo.replace(rm, '');
else return await msg.reply("Utilisateur non sudo\n Numéros sudo : " + global.sudo );
...
.then(data => 
      { 
   console.log(data);
   return msg.reply(`*${user} Supprimé avec succès.*\nNuméros sudo : ${newVarValue}`);
      })
  
.catch(error => { return msg.error('Erreur en ajoutant un sudo \n'+ error);      })
...
.catch(error => msg.error('Erreur de variable\n'+ error));
...
async(Suhail, msg , text,{ isCreator }) => {
     ...
     if (!isCreator) return msg.reply(tlang().owner);
if (!text) return msg.reply (`Nom de la variable\n*E.x : ${prefix}setvar CAPTION: Developpé par Crazy237*`);
const headers = 
        {
                 'Accept': 'application/vnd.heroku+json; version=3',
                 'Authorization': `Bearer ${authToken}`,
...
fetch(`https://api.heroku.com/apps/${appName}/config-vars`,
  ...
  .then(response => response.json())
  .then(data => {  return msg.reply(`*${varName} mise à jour avec succès.*\n${varName}  :  ${newVarValue}`);   })
  .catch(error => msg.error('Erreur d\' ajout de variable\n'+error));
  });
//-----------------------------------------------------------------------------------
...
smd({
    ...
    async(Suhail, msg , text,{ isCreator }) => {
 if (!isCreator) return msg.reply(tlang().owner);
if (!text) return msg.reply (`Nom de la variable\n*Exemple : ${prefix}setvar CAPTION: Developpé par Crazy237*`);
const headers = {
  'Accept': 'application/vnd.heroku+json; version=3',
  'Authorization': `Bearer ${authToken}`,
  'Content-Type': 'application/json'
...
if (!newVarValue) return msg.reply (`Donnez la valeur après le ':' \n*Example : ${prefix}setvar AUTO_READ_STATUS:true*`);       
fetch(`https://api.heroku.com/apps/${appName}/config-vars`, {
  method: 'GET',
  headers 
}) 
  .then(response => {
            ...
            else { throw new Error(`Variables introuvables. Status: ${response.status}`); }
  })
  .then(data => {
